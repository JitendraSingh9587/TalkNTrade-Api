...
const sendOTP = async (otpData) => {
  ...
  if (finalPurpose === 'REGISTER' && user) {
    ...
    const error = new Error('User already exists with this email. Use LOGIN purpose instead.');
  ...
  if (finalPurpose === 'LOGIN' && !user) {
    ...
    const error = new Error('User not found with this email. Use REGISTER purpose instead.');
...
const verifyOTP = async (verifyData) => {
  const { email, otp, purpose = null, channel = null, userData = null } = verifyData;
  ...
  if (finalPurpose === 'REGISTER' && otpRecord.purpose === 'REGISTER') {
    if (!userData || !userData.name || !userData.mobile || !userData.password) {
      const error = new Error('User data (name, mobile, password) is required for registration');
      error.statusCode = 400;
      throw error;
    }
